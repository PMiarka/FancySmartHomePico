...
bus = SMBus(1,sda=Pin(2), scl=Pin(3), freq=400000) 

#try:
sensor = BME680(I2C_ADDR_PRIMARY,bus)

KEY_READ_ALL_SENSORS_DATA = "READ_ALL"
KEY_SET_COMPENSATION = "COMPENSATION"

def ParseData(decodedData, uart):
    print("ParseData " + decodedData)
    result = decodedData.find(KEY_SET_COMPENSATION) > -1
    result2 = decodedData.find(KEY_READ_ALL_SENSORS_DATA) > -1
    print(result)
    print(result2)
    if(decodedData.find(KEY_SET_COMPENSATION) > -1):
        dataSplitted = decodedData.split("/")
        print(dataSplitted)
        sensor.set_temp_offset(float(dataSplitted[1]))
    elif(decodedData.find(KEY_READ_ALL_SENSORS_DATA) > -1):
        sensor.get_sensor_data()
        print(str(sensor.data.temperature)+"|"+str(sensor.data.pressure)+"|"+str(sensor.data.humidity))
        uart.write(str(sensor.data.temperature) + "|")
        uart.write(str(sensor.data.pressure) + "|")
        uart.write(str(sensor.data.humidity))
...
while True:
    ...
    while True:
        if uart.any() > 0:
            rxData = uart.read()
            print("uart.any() > 0 "+ str(rxData))
            time.sleep_ms(20)
            print(rxData)
#             uart.write(rxData)
            print("before led " + rxData.decode("utf-8"))
            ParseData(rxData.decode("utf-8"), uart)
#            if(rxData == b'999'):
#                print("led")
#                 led.toggle()
